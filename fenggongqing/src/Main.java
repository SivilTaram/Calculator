...
public class Main {
    ...
    public static String MakeFormula(){
        ...
        int count = (int) (Math.random() * 2) + 1; // 制作公式
        int start = 0;
        int number1 = (int) (Math.random() * 99) + 1;
        build.append(number1);
        while (start <= count){
            int operation = (int) (Math.random() * 3); // generate operator 生成运算符
    ...
    public static String Solve(String formula){
        Stack<String> tempStack = new Stack<>();//Store number or operator 商店编号或操作员
        Stack<Character> operatorStack = new Stack<>();//Store operator  专卖店经营者
